...
@section('content')
    <div class="card mx-4 mt-3">
        <div class="d-flex justify-content-between card-header ">
            <h5 class="text-dark fw-bold">Pendataan Baptis</h5>
        </div>
                    ...
                    <table id="myTable" class="table align-items-center mb-0">
                        <thead>
                            <tr>
                                ...
                                <th class="text-uppercase text-xs font-weight-bolder">Nama Pendaftar Baptis</th>
                            ...
                            @foreach ($data as $index => $item)
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex px-2 py-1">
                                            <div class="d-flex flex-column justify-content-center">
                                                @if ($item->status_baptis == 'Mendaftar')
                                                    <form id="konfirmasi-baptis-{{ $item->id }}"
                                                        action="/pendataan-baptis/{{ $item->id }}" method="POST"
                                                        role="form text-left">
                                                        @csrf
                                                        @method('PUT')
                                                        <input type="hidden" name="action_type" value="konfirmasi_baptis">
                                                ...
                                                @elseif($item->status_baptis == 'Dikonfirmasi')
                                                    <form id="sudah-baptis-{{ $item->id }}"
                                                        action="/pendataan-baptis/{{ $item->id }}" method="POST"
                                                        role="form text-left"
                                                        onsubmit="event.preventDefault(); confirmSudahBaptis({{ $item->id }})">
                                                        @csrf
                                                        @method('PUT')
                                                        <input type="hidden" name="action_type" value="sudah_baptis">
    ...
    <script>
        function confirmSudahBaptis(id) {
            Swal.fire({
                title: "Konfirmasi Jemaat Baptis",
                text: "Apakah jemaat sudah melaksanakan baptis?",
                icon: "question",
                ...
                if (result.isConfirmed) {
                    document.getElementById('sudah-baptis-' + id).submit();
                }
            });
        }
    </script>
